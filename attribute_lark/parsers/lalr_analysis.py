...
from .grammar_analysis import GrammarAnalyzer, Terminal, LR0ItemSet, State
...
class ParseTableBase(Generic[StateT]):
    ...
    def serialize(self, memo):
        ...
        states = {
            state: {
                tokens.get(token): (
                    (1, arg.serialize(memo), ast) if action is Reduce else (0, arg, ast)
                )
    ...
    def deserialize(cls, data, memo):
        ...
        states = {
            state: {
                tokens[token]: (
                    (Reduce, Rule.deserialize(arg, memo), ast)
                    if action == 1
                    else (Shift, arg, ast)
                )
                for token, (action, arg, ast) in actions.items()
...
class IntParseTable(ParseTableBase[int]):
    ...
    def from_ParseTable(cls, parse_table: ParseTable):
        ...
        for s, la in parse_table.states.items():
            ...
            la = {
                k: (v[0], state_to_idx[v[1]], v[2]) if v[0] is Shift else v
                for k, v in la.items()
            }
...
class LALR_Analyzer(GrammarAnalyzer):
    ...
    def compute_lr0_states(self) -> None:
        ...
        def step(state: LR0ItemSet) -> Iterator[LR0ItemSet]:
            ...
            msgs = []
            for _, ctx_term_collision in classify(
                [s for s in d if s.is_term], lambda s: s.name
            ).items():
                if len(ctx_term_collision) > 1:
                    msg = (
                        "Contextual lookahead collision between the following rules: %s"
    ...
    def compute_lalr1_states(self) -> None:
        ...
        for itemset in self.lr0_itemsets:
            actions: Dict[Symbol, Tuple] = {
                la: (Shift, next_state.closure, la.ast)
