...
class RuleTreeToText(Transformer):
    ...
    def attributed_expansion(self, x):
        ...
        assert _syn_attribute is None, (
            "Synthesized attribute defined outside of attribute_expansion"
        )
...
class CheckName(ast.NodeVisitor):
    ...
    def visit_Name(self, node):
        assert node.id != self.name, (
            f"Invalid use of variable {self.name} in attribute expression"
        )
...
class TransformAttribute(ast.NodeTransformer):
    ...
    def visit_Name(self, node):
        if node.id == "inh":
            ...
            return ast.Subscript(
                value=ast.Name(id="stack", ctx=ast.Load()),
                slice=ast.Constant(value=inherited_idx),
                ctx=ast.Load(),
            )
        else:
            return node
    ...
    def visit_Subscript(self, node):
        if isinstance(node.value, ast.Name) and node.value.id == "syn":
            assert self.offsets, (
                "syn used in expression on an empty stack of attributes"
            )
            assert isinstance(node.slice, ast.Constant), (
                "syn used in expression with a non constant index"
            )
            assert 1 <= node.slice.value <= len(self.offsets), (
                "syn used in expression outside of the stack of attributes: %s"
                % ast.unparse(node)
            )
            new_slice = self.offsets[node.slice.value - 1] - (self.offsets[-1] + 1)
            return ast.Subscript(
                value=ast.Name(id="stack", ctx=ast.Load()),
                slice=ast.Constant(value=new_slice),
                ctx=ast.Load(),
            )
        else:
            return ast.Subscript(
...
class Grammar:
    ...
    def compile(
    ...
    ) -> Tuple[List[TerminalDef], List[Rule], List[str], Optional[ast.Module]]:
        ...
        assert len(rules) == len({name for name, _t, _o in rules}), (
        ...
        for rule_content in rules:
            ...
            for i, (expansion, alias, syn_attribute) in enumerate(expansions):
                ...
                marker_expansion, marker_rules, offsets = transform_expansion(
                    expansion, f"_#{name}#{i}", exp_options
                )
        ...
        python_header = (
            ast.parse(self._python_header, mode="exec") if self._python_header else None
        )
...
stdlib_loader = FromPackageLoader("attribute_lark", IMPORT_PATHS)
...
class PrepareGrammar(Transformer_InPlace):
    ...
    def syn_attribute(self, *children):
        ...
        if len(children) == 0:
            ...
            child = children[0]
            assert isinstance(child, Token) and child.type == "ATT_EXPR", (
                "Invalid token type for synthesized attribute"
            )
...
def find_grammar_errors(
    ...
    #         e.interactive_parser.as_immutable(),
    #         lambda p: Token("_NL", "(.*?)") in p.choices(),
    #     )
    ...
    _tree, _ = _get_parser().parse(text + "\n", start)  # , on_error=on_error)
        ...
        for stmt in tree.children:
            if stmt.data == "import":
                ...
                try:
                    ...
                    assert base_path == import_base_path, (
